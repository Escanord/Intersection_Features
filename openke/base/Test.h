...
void getTestEdges(
	...
	for (INT i = 0; i < tripleTotal; ++i) {
        ...
        batch_h[tripleTotal + i * 2] = tripleList[i].h;
		batch_t[tripleTotal + i * 2] = entityTotal + tripleList[i].r;
		batch_h[tripleTotal + i * 2 + 1] = entityTotal + tripleList[i].r;
		batch_t[tripleTotal + i * 2 + 1] = tripleList[i].t;
	}
}
