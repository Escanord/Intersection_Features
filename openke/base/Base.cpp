...
void getEdgeIndices(
	...
	for (INT i = 0; i < trainTotal; ++i) {
		batch_h[i] = trainList[i].h;
		batch_t[i] = trainList[i].t;
		batch_h[trainTotal + i * 2] = trainList[i].h;
		batch_t[trainTotal + i * 2] = entityTotal + trainList[i].r;
		batch_h[trainTotal + i * 2 + 1] = entityTotal + trainList[i].r;
		batch_t[trainTotal + i * 2 + 1] = trainList[i].t;
	}
}
