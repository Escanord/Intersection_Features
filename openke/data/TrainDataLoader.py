...
class TrainDataLoader(object):
	...
	def __init__(self,
		...
		self.read()
		self.hash_dataset = HashDataset(self.links, self.entTotal + self.relTotal, max_hash_hops=self.max_hash_hops, hll_p=hll_p,  minhash_num_perm=minhash_num_perm)
	...
	def load_links(self):
		full_list_h = np.zeros(self.tripleTotal * 3, dtype=np.int64)
		full_list_t = np.zeros(self.tripleTotal * 3, dtype=np.int64)
	...
	#get random batch_size samples each time
	def sampling(self):
		self.lib.sampling(
			self.batch_h_addr,
			self.batch_t_addr,
		...
		links = torch.from_numpy(np.vstack([self.batch_h, self.batch_t]).T).to(self.device)
		relationships = torch.from_numpy(np.add(self.batch_r, self.entTotal)).to(self.device)
